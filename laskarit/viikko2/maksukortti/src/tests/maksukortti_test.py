...
class TestMaksukortti(unittest.TestCase):
    ...
    def test_negatiivinen_lataus_ei_muuta_kortin_arvoa(self):
        self.kortti.lataa_rahaa(-1000)

        self.assertEqual(str(self.kortti), "Kortilla on rahaa 10.00 euroa")
    
    def test_edullisen_osto_onnistuu_jos_sama_rahamäärä_kortilla(self):
        kortti = Maksukortti(250)
        kortti.syo_edullisesti()

        self.assertEqual(str(kortti), "Kortilla on rahaa 0.00 euroa")
    
    def test_maukkaan_osto_onnistuu_jos_sama_rahamäärä_kortilla(self):
